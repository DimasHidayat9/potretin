const video = document.getElementById('webcam');
    ...
    // null = fill the next empty slot(s) in sequence; a number = replace just that one slot
    let galleryFillTargetIdx = null;
    ...
    const cameraSkeletonEl = document.getElementById('camera-skeleton');
    ...
    const welcomeStageMode = document.getElementById('welcome-stage-mode');
    const galleryModePlaceholder = document.getElementById('gallery-mode-placeholder');
    const poseIntervalBlock = document.getElementById('pose-interval-block');
    const autoCropBlock = document.getElementById('auto-crop-block');
    // 'camera' = potret langsung lewat webcam; 'gallery' = isi strip dari foto yang sudah ada
    let sessionMode = 'camera';
    ...
    // Dipakai tombol "Tampilkan QR" — QR sekarang cuma dirender saat diminta,
    // bukan otomatis lagi tiap kali strip/grid diunduh.
    let lastExportedDataUrl = null;
    const qrRevealBtn = document.getElementById('qr-reveal-btn');
    ...
    // #photo-list di HTML defaultnya "flex flex-col" (susun vertikal) — itu pas
    // buat strip4/strip6, tapi kalau layout "Grid 2×2" dipilih, live preview saat
    // motret harus ikut disusun 2 kolom, supaya konsisten sama hasil unduhan grid.
    // Sebelumnya fungsi ini nggak ada, jadi kotak slot foto tetap kelihatan strip
    // vertikal walau "Grid 2×2" sudah dipilih di menu.
    function applyPhotoListLayoutClass() {
      const preset = LAYOUT_PRESETS[selectedLayout] || LAYOUT_PRESETS.strip4;
      const isGridLayout = preset.exportMode === 'grid';
      photoList.classList.toggle('grid', isGridLayout);
      photoList.classList.toggle('grid-cols-2', isGridLayout);
      photoList.classList.toggle('flex', !isGridLayout);
      photoList.classList.toggle('flex-col', !isGridLayout);
    }
    ...
    // Re-triggers a CSS entrance animation on an element even if its class
    // never left, by forcing a reflow between remove/add.
    function replayStageAnim(el) {
      if (!el) return;
      el.classList.remove('stage-animate');
      void el.offsetWidth;
      el.classList.add('stage-animate');
    }
    ...
    function goToLayoutStage() {
      ...
      replayStageAnim(welcomeStageLayout);
    }
    function backToStartStage() {
      ...
      replayStageAnim(welcomeStageStart);
    }
    ...
    function chooseLayout(layoutKey) {
      ...
      applyPhotoListLayoutClass();
      photoList.innerHTML = buildPlaceholderHTML(POSE_COUNT);
      updateStatusUI(0);
      highlightRecommendedExport(preset.exportMode);
      // Layout baru dipilih (mis. Strip -> Grid 2x2): render ulang gambar preview
      // asli (canvas), bukan cuma mockup HTML-nya — tanpa baris ini, gambar lama
      // (hasil render layout sebelumnya, yang sudah nempel sebagai <img> sejak
      // load halaman) tetap kepasang di layar walau layout sudah ganti.
      refreshLiveFramedPreview();

      // Layout sudah dipilih — sebelum kamera dinyalakan, tanya dulu mau isi
      // fotonya lewat kamera langsung atau upload dari galeri.
      goToModeStage();
    }
    ...
    function goToModeStage() {
      welcomeStageLayout.classList.add('hidden');
      welcomeStageLayout.classList.remove('flex');
      welcomeStageMode.classList.remove('hidden');
      welcomeStageMode.classList.add('flex');
      replayStageAnim(welcomeStageMode);
    }
    function backToLayoutStage() {
      welcomeStageMode.classList.add('hidden');
      welcomeStageMode.classList.remove('flex');
      welcomeStageLayout.classList.remove('hidden');
      welcomeStageLayout.classList.add('flex');
      replayStageAnim(welcomeStageLayout);
    }

    // Menyesuaikan UI studio berdasarkan mode yang dipilih — mode kamera dan mode
    // galeri sengaja TIDAK dicampur: kontrol khusus kamera (feed video, jeda hitung
    // mundur, auto-crop) disembunyikan total di mode galeri, dan sebaliknya tombol
    // upload besar cuma tampil di mode galeri.
    function applySessionModeUI(mode) {
      const isGallery = mode === 'gallery';
      video.classList.toggle('hidden', isGallery);
      galleryModePlaceholder.classList.toggle('hidden', !isGallery);
      galleryModePlaceholder.classList.toggle('flex', isGallery);
      snapBtn.classList.toggle('hidden', isGallery);
      galleryUploadBtn.classList.toggle('hidden', !isGallery);
      galleryUploadBtn.classList.toggle('flex', isGallery);
      if (poseIntervalBlock) poseIntervalBlock.classList.toggle('hidden', isGallery);
      if (autoCropBlock) autoCropBlock.classList.toggle('hidden', isGallery);
      if (isGallery) disableAutoFrame();
    }

    function chooseSessionMode(mode) {
      sessionMode = (mode === 'gallery') ? 'gallery' : 'camera';
      applySessionModeUI(sessionMode);
      ...
      if (sessionMode === 'camera') {
        ...
        showToast(`Sesi dimulai · Potret Langsung`, '🎬');
      } else {
        showToast(`Sesi dimulai · Upload dari Galeri`, '🖼️');
        // Langsung buka dialog pilih file begitu layar studio kelihatan, biar
        // orang gak perlu cari-cari tombolnya lagi.
        setTimeout(() => openGalleryPicker(), 450);
      }
    ...
    // ============ ONBOARDING TOUR ============
    // Penjelasan fitur langkah-demi-langkah, muncul otomatis sekali di kunjungan
    // pertama (sebelum user sempat asal klik), dan bisa dibuka lagi lewat
    // "Panduan Fitur" di Menu Lainnya.
    const ONBOARDING_SEEN_KEY = 'jepretin_onboarding_seen_v1';
    const ONBOARDING_SLIDES = [
      {
        icon: 'fa-solid fa-camera-retro', iconBg: 'bg-[var(--brass)]/15', iconColor: 'text-[var(--brass-soft)]',
        title: 'Selamat Datang di Jepretin!',
        text: 'Studio foto instan yang bisa kamu pakai sendiri. Potret, pilih gaya, langsung simpan dalam hitungan detik. Yuk kenalan dulu sama fiturnya.'
      },
      {
        icon: 'fa-solid fa-swatchbook', iconBg: 'bg-sky-500/15', iconColor: 'text-sky-600',
        title: '1. Pilih Bentuk Cetak',
        text: 'Strip 4 atau 6 pose ala photobooth klasik, atau Grid 2×2 yang kotak rapi buat medsos. Tinggal pilih sesuai selera.'
      },
      {
        icon: 'fa-solid fa-crosshairs', iconBg: 'bg-emerald-500/15', iconColor: 'text-emerald-600',
        title: '2. Potret atau Upload',
        text: 'Potret langsung pakai kamera, lengkap dengan hitung mundur & Auto-Crop yang otomatis nge-track posisi kamu. Nggak sempat foto langsung? Upload aja dari galeri.'
      },
      {
        icon: 'fa-brands fa-instagram', iconBg: 'bg-[var(--crimson)]/15', iconColor: 'text-[var(--crimson-soft)]',
        title: '3. Gaya & Bagikan',
        text: '18 tema bingkai, GIF animasi, sampai bingkai siap upload buat Instagram, semua bisa diatur sebelum disimpan.'
      },
      {
        icon: 'fa-solid fa-clock-rotate-left', iconBg: 'bg-[var(--brass)]/15', iconColor: 'text-[var(--brass-soft)]',
        title: '4. Tersimpan Otomatis',
        text: 'Semua hasil otomatis masuk Riwayat di tab Galeri. Mau suasana lebih santai? Ganti musik latar lewat ikon sliders di pojok atas.'
      }
    ];
    let tourIndex = 0;
    const tourModal = document.getElementById('onboarding-tour-modal');
    const tourIconEl = document.getElementById('tour-slide-icon');
    const tourTitleEl = document.getElementById('tour-slide-title');
    const tourTextEl = document.getElementById('tour-slide-text');
    const tourDotsEl = document.getElementById('tour-dots');
    const tourNextBtn = document.getElementById('tour-next-btn');

    function renderTourSlide() {
      const slide = ONBOARDING_SLIDES[tourIndex];
      if (!slide || !tourIconEl) return;
      tourIconEl.className = `w-16 h-16 rounded-2xl flex items-center justify-center text-2xl shrink-0 tour-icon-pop ${slide.iconBg} ${slide.iconColor}`;
      tourIconEl.innerHTML = `<i class="${slide.icon}"></i>`;
      tourTitleEl.textContent = slide.title;
      tourTextEl.textContent = slide.text;
      tourTitleEl.parentElement.classList.remove('tour-content-in');
      void tourTitleEl.parentElement.offsetWidth;
      tourTitleEl.parentElement.classList.add('tour-content-in');
      tourDotsEl.innerHTML = ONBOARDING_SLIDES.map((_, i) =>
        `<span class="tour-dot${i === tourIndex ? ' is-active' : ''}"></span>`
      ).join('');
      const isLast = tourIndex === ONBOARDING_SLIDES.length - 1;
      tourNextBtn.querySelector('span').textContent = isLast ? 'Mengerti, Yuk Mulai!' : 'Lanjut';
    }
    ...
    function tourNext() {
      if (tourIndex < ONBOARDING_SLIDES.length - 1) {
        tourIndex++;
        renderTourSlide();
      } else {
        closeOnboardingTour();
    ...
    function skipOnboardingTour() { closeOnboardingTour(); }

    // Trigger otomatis di kunjungan pertama — modal ini z-[99], jadi tetap
    // ketutup splash screen (z-[100]) sampai splash-nya kelar sendiri.
    (function maybeAutoStartTour() {
      try {
        if (!localStorage.getItem(ONBOARDING_SEEN_KEY)) startOnboardingTour();
      } catch (e) { /* localStorage unavailable — skip tour, jangan sampai app rusak */ }
    })();

    // Daftarkan service worker (app-shell offline + syarat "installable" PWA).
    // Dibungkus try/catch dan pengecekan fitur karena file ini juga dibuka lewat
    // file:// saat development, yang tidak mendukung service worker sama sekali.
    if ('serviceWorker' in navigator && (location.protocol === 'https:' || location.hostname === 'localhost')) {
      window.addEventListener('load', () => {
        navigator.serviceWorker.register('./sw.js').catch(() => {
          // Registrasi gagal (mis. hosting statis yang tidak mengizinkan SW) — biarkan
          // saja, app tetap jalan normal cuma tanpa dukungan offline/installable penuh.
        });
      });
    }
    ...
    // Dock ini cuma relevan buat "loncat antar halaman" — jadi disembunyikan
    // otomatis setiap kali layar penuh (pilih layout/bingkai, modal foto, dst.)
    // sedang terbuka, dan hanya muncul lagi di halaman utama (landing) atau
    // studio kamera yang sudah aktif. Dipantau via MutationObserver supaya
    // otomatis update dari mana pun overlay itu dibuka/ditutup, tanpa perlu
    // menambal tiap fungsi show/hide modal satu-satu.
    function computeNavShouldHide() {
      const fullscreenOverlays = [
        frameModal, igModal, battleModal, previewModal,
        document.getElementById('history-modal'),
        document.getElementById('mockup-modal'),
      ];
      const overlayOpen = fullscreenOverlays.some((m) => m && !m.classList.contains('hidden'));
      // welcomeScreen sits behind landingScreen (lower z-index) and, unless it was
      // explicitly closed, keeps style.display unset — so only count it as "open"
      // when it's actually the front-most thing on screen, not just present in the DOM.
    ...
    function updateBottomNavVisibility() {
      if (!bottomNav) return;
      bottomNav.classList.toggle('nav-hidden', computeNavShouldHide());
    }
    (function watchNavVisibility() {
      const watched = [
        frameModal, igModal, battleModal, previewModal, welcomeScreen, landingScreen,
        document.getElementById('history-modal'),
        document.getElementById('mockup-modal'),
      ];
    ...
    async function initCamera(deviceId) {
      ...
      // Tampilkan skeleton hanya untuk permintaan izin/inisialisasi pertama —
      // saat ganti kamera (deviceId sudah ada stream sebelumnya) feed lama masih
      // kelihatan jadi nggak perlu skeleton menutupinya.
      const showSkeleton = cameraSkeletonEl && !currentStream;
      if (showSkeleton) {
        cameraSkeletonEl.classList.remove('hidden');
        cameraSkeletonEl.classList.add('flex');
      }
      try {
        if (currentStream) {
        ...
        if (showSkeleton) {
          cameraSkeletonEl.classList.add('hidden');
          cameraSkeletonEl.classList.remove('flex');
        }
      }
    }
    ...
    async function startCaptureSequence() {
      if (isCapturing || isRetaking || sessionMode !== 'camera') return;
      isCapturing = true;
      capturedImages = [];
      capturedBursts = [];
      favoriteIndex = null;
      ...
      if (qrRevealBtn) qrRevealBtn.classList.add('hidden');
      qrPanel.classList.add('hidden');
      lastExportedDataUrl = null;
    ...
    /* ---------- Upload dari Galeri — "pinjam" bingkai/strip pakai foto yang sudah ada, tanpa kamera ---------- */
    function openGalleryPicker(idx) {
      if (isCapturing || isRetaking) return;
      // null = isi slot kosong berikutnya secara berurutan; angka = ganti satu slot itu saja
      galleryFillTargetIdx = (typeof idx === 'number') ? idx : null;
      galleryInput.multiple = (galleryFillTargetIdx === null);
      galleryInput.value = ''; // biar bisa pilih file yang sama lagi kalau perlu
      galleryInput.click();
    }
    ...
    // Menggambar foto upload lewat pipeline canvas yang sama dengan capture kamera —
    // diberi filter yang lagi aktif — supaya foto dari galeri nyatu sama strip.
    // srcX/Y/W/H menentukan area sumber yang diambil dari foto asli; dulu ini selalu
    // otomatis cover-fit dari tengah (lihat coverFitRect), sekarang areanya datang
    // dari hasil geser & zoom user di modal "Atur Posisi Foto" (lihat di bawah).
    function drawGalleryRectToDataUrl(img, srcX, srcY, srcW, srcH) {
      const ctx = captureCanvas.getContext('2d');
      const w = video.videoWidth || 1280, h = video.videoHeight || 960;
      captureCanvas.width = w;
      captureCanvas.height = h;
    ...
    async function handleGalleryFiles(fileList) {
      ...
      if (typeof galleryFillTargetIdx === 'number') {
        // Ganti satu slot yang sudah terisi — sama seperti retake manual, tapi sumbernya galeri.
        const idx = galleryFillTargetIdx;
        galleryFillTargetIdx = null;
        queueGalleryAdjust([{ file: files[0], idx }], { bulk: false });
        return;
      }

      // Isi mulai dari slot kosong pertama — bisa dicampur sama hasil kamera, atau
      // langsung isi semua slot dari galeri kalau mau skip kamera sepenuhnya.
      const startIdx = capturedImages.length;
      const remaining = POSE_COUNT - startIdx;
      const toUse = files.slice(0, Math.max(remaining, 0));
      if (!toUse.length) { showToast('Strip sudah penuh — reset dulu buat ganti semua.', 'ℹ️'); return; }

      clearIdleTimer();
      favoriteIndex = null;
      celebrated = false;
      const entries = toUse.map((file, i) => ({ file, idx: startIdx + i }));
      queueGalleryAdjust(entries, { bulk: true });
    }
    ...
    /* ---------- Atur Posisi Foto Galeri — geser & zoom biar foto pas & simetris di
       tengah bingkai sebelum dipasang, menggantikan auto-crop cover-fit yang selalu
       memotong dari tengah tanpa kendali user. Foto diproses satu per satu lewat
       antrean supaya upload banyak foto sekaligus tetap bisa diatur satu-satu. ---------- */
    const galleryAdjustModal = document.getElementById('gallery-adjust-modal');
    const galleryAdjustFrame = document.getElementById('gallery-adjust-frame');
    const galleryAdjustImgEl = document.getElementById('gallery-adjust-img');
    const galleryAdjustZoomInput = document.getElementById('gallery-adjust-zoom');

    let galleryAdjustQueue = [];
    let galleryAdjustMeta = { bulk: false, total: 0, applied: 0 };
    let galleryAdjustCurrentImg = null;
    let galleryAdjustCurrentIdx = null;
    let galleryAdjustGeom = null; // { frameW, frameH, naturalW, naturalH, baseScale, userScale, translateX, translateY }
    let galleryAdjustDrag = null;

    function queueGalleryAdjust(entries, meta) {
      galleryAdjustQueue = entries.slice();
      galleryAdjustMeta = { bulk: !!meta.bulk, total: entries.length, applied: 0 };
      if (galleryAdjustMeta.bulk) {
        galleryUploadBtn.disabled = true;
        snapBtn.disabled = true;
        poseCountSelect.disabled = true;
        intervalSelect.disabled = true;
      }
      processNextGalleryAdjust();
    }

    async function processNextGalleryAdjust() {
      if (!galleryAdjustQueue.length) {
        finishGalleryAdjustQueue();
        return;
      }
      const entry = galleryAdjustQueue.shift();
      try {
        const dataUrl = await readFileAsDataUrl(entry.file);
        const img = await loadImageEl(dataUrl);
        openGalleryAdjustModal(img, entry.idx);
      } catch (e) {
        showToast('Gagal memuat foto itu, coba yang lain ya.', '⚠️');
        processNextGalleryAdjust();
      }
    }

    function finishGalleryAdjustQueue() {
      if (galleryAdjustMeta.bulk) {
        galleryUploadBtn.disabled = false;
        snapBtn.disabled = false;
        poseCountSelect.disabled = false;
        intervalSelect.disabled = false;
        if (galleryAdjustMeta.applied > 0) {
          showToast(`${galleryAdjustMeta.applied} foto dari galeri berhasil dipasang!`, '🖼️');
          if (capturedImages.length === POSE_COUNT) {
            triggerGrandReveal();
            if (!battleDone && capturedImages.length >= 2) {
              setTimeout(() => startPhotoBattle(), 1200);
            }
          }
        }
      } else if (galleryAdjustMeta.applied > 0) {
        showToast(`Pose diganti pakai foto dari galeri!`, '🖼️');
      }
      galleryAdjustMeta = { bulk: false, total: 0, applied: 0 };
    }

    function openGalleryAdjustModal(img, idx) {
      galleryAdjustCurrentImg = img;
      galleryAdjustCurrentIdx = idx;
      // Rasio bingkai adjust disamakan dengan rasio kanvas capture (video atau
      // fallback 4:3) supaya pratinjau yang dilihat user = hasil akhir yang dipasang.
      const ratioW = video.videoWidth || 1280, ratioH = video.videoHeight || 960;
      if (galleryAdjustFrame) galleryAdjustFrame.style.aspectRatio = ratioW + ' / ' + ratioH;
      if (galleryAdjustModal) {
        galleryAdjustModal.classList.remove('hidden');
        galleryAdjustModal.classList.add('flex');
      }
      // Baca ukuran render setelah modal ditampilkan & aspect-ratio diterapkan.
      requestAnimationFrame(() => {
        const frameW = galleryAdjustFrame.clientWidth || 320;
        const frameH = galleryAdjustFrame.clientHeight || (frameW * ratioH / ratioW);
        const baseScale = Math.max(frameW / img.naturalWidth, frameH / img.naturalHeight);
        galleryAdjustGeom = {
          frameW, frameH,
          naturalW: img.naturalWidth, naturalH: img.naturalHeight,
          baseScale, userScale: 1,
          translateX: 0, translateY: 0
        };
        if (galleryAdjustImgEl) galleryAdjustImgEl.src = img.src;
        if (galleryAdjustZoomInput) galleryAdjustZoomInput.value = 100;
        applyGalleryAdjustTransform();
      });
    }

    // Menerapkan transform tampilan (posisi + zoom) ke <img> pratinjau, dengan
    // translate dijepit (clamp) supaya foto selalu menutupi penuh area bingkai —
    // "simetris di tengah" jadi posisi default (translate 0,0) yang selalu bisa
    // dikembalikan lewat tombol Tengahkan.
    function applyGalleryAdjustTransform() {
      const g = galleryAdjustGeom;
      if (!g || !galleryAdjustImgEl) return;
      const totalScale = g.baseScale * g.userScale;
      const dispW = g.naturalW * totalScale;
      const dispH = g.naturalH * totalScale;
      const maxOffsetX = Math.max(0, (dispW - g.frameW) / 2);
      const maxOffsetY = Math.max(0, (dispH - g.frameH) / 2);
      g.translateX = Math.max(-maxOffsetX, Math.min(maxOffsetX, g.translateX));
      g.translateY = Math.max(-maxOffsetY, Math.min(maxOffsetY, g.translateY));
      galleryAdjustImgEl.style.width = dispW + 'px';
      galleryAdjustImgEl.style.height = dispH + 'px';
      galleryAdjustImgEl.style.transform =
        `translate(-50%, -50%) translate(${g.translateX}px, ${g.translateY}px)`;
    }

    function onGalleryAdjustZoom(val) {
      if (!galleryAdjustGeom) return;
      galleryAdjustGeom.userScale = Math.max(1, Math.min(3, Number(val) / 100));
      applyGalleryAdjustTransform();
    }

    function resetGalleryAdjust() {
      if (!galleryAdjustGeom) return;
      galleryAdjustGeom.userScale = 1;
      galleryAdjustGeom.translateX = 0;
      galleryAdjustGeom.translateY = 0;
      if (galleryAdjustZoomInput) galleryAdjustZoomInput.value = 100;
      applyGalleryAdjustTransform();
    }

    function bindGalleryAdjustDrag() {
      if (!galleryAdjustFrame) return;
      galleryAdjustFrame.addEventListener('pointerdown', (e) => {
        if (!galleryAdjustGeom) return;
        try { galleryAdjustFrame.setPointerCapture(e.pointerId); } catch (err) {}
        galleryAdjustDrag = {
          pointerId: e.pointerId, startX: e.clientX, startY: e.clientY,
          startTX: galleryAdjustGeom.translateX, startTY: galleryAdjustGeom.translateY
        };
      });
      galleryAdjustFrame.addEventListener('pointermove', (e) => {
        if (!galleryAdjustDrag || galleryAdjustDrag.pointerId !== e.pointerId || !galleryAdjustGeom) return;
        galleryAdjustGeom.translateX = galleryAdjustDrag.startTX + (e.clientX - galleryAdjustDrag.startX);
        galleryAdjustGeom.translateY = galleryAdjustDrag.startTY + (e.clientY - galleryAdjustDrag.startY);
        applyGalleryAdjustTransform();
      });
      const endDrag = () => { galleryAdjustDrag = null; };
      galleryAdjustFrame.addEventListener('pointerup', endDrag);
      galleryAdjustFrame.addEventListener('pointercancel', endDrag);
      // Scroll wheel buat zoom cepat di desktop — pelengkap slider, bukan pengganti.
      galleryAdjustFrame.addEventListener('wheel', (e) => {
        if (!galleryAdjustGeom || !galleryAdjustZoomInput) return;
        e.preventDefault();
        const next = Math.max(100, Math.min(300, Number(galleryAdjustZoomInput.value) + (e.deltaY > 0 ? -5 : 5)));
        galleryAdjustZoomInput.value = next;
        onGalleryAdjustZoom(next);
      }, { passive: false });
    }
    bindGalleryAdjustDrag();

    function closeGalleryAdjustModalInternal() {
      if (galleryAdjustModal) {
        galleryAdjustModal.classList.add('hidden');
        galleryAdjustModal.classList.remove('flex');
      }
      galleryAdjustCurrentImg = null;
      galleryAdjustGeom = null;
      galleryAdjustCurrentIdx = null;
    }

    function confirmGalleryAdjust() {
      const g = galleryAdjustGeom;
      const img = galleryAdjustCurrentImg;
      const idx = galleryAdjustCurrentIdx;
      if (!g || !img || idx == null) { closeGalleryAdjustModalInternal(); processNextGalleryAdjust(); return; }
      const totalScale = g.baseScale * g.userScale;
      const dispW = g.naturalW * totalScale, dispH = g.naturalH * totalScale;
      const imgLeft = (g.frameW - dispW) / 2 + g.translateX;
      const imgTop = (g.frameH - dispH) / 2 + g.translateY;
      const srcX = Math.max(0, Math.min(g.naturalW, -imgLeft / totalScale));
      const srcY = Math.max(0, Math.min(g.naturalH, -imgTop / totalScale));
      const srcW = Math.min(g.frameW / totalScale, g.naturalW - srcX);
      const srcH = Math.min(g.frameH / totalScale, g.naturalH - srcY);
      const dataUrl = drawGalleryRectToDataUrl(img, srcX, srcY, srcW, srcH);
      capturedImages[idx] = dataUrl;
      capturedBursts[idx] = null; // foto upload nggak punya gerakan burst — GIF export otomatis fallback ke frame diam
      galleryAdjustMeta.applied++;
      renderStripPreview();
      closeGalleryAdjustModalInternal();
      processNextGalleryAdjust();
    }

    function cancelGalleryAdjust() {
      closeGalleryAdjustModalInternal();
      processNextGalleryAdjust();
    }
    ...
    function renderStripPreview() {
      ...
      capturedImages.forEach((src, idx) => {
        ...
        if (sessionMode === 'gallery') {
          retakeBtn.setAttribute('aria-label', `Ganti pose ${idx + 1} dari galeri`);
          retakeBtn.title = "Ganti pakai foto lain dari galeri";
          retakeBtn.innerHTML = '<i class="fa-solid fa-image"></i>';
          retakeBtn.onclick = (e) => { e.stopPropagation(); openGalleryPicker(idx); };
        } else {
      ...
      if (allDone) {
        ...
        if (qrRevealBtn) qrRevealBtn.classList.remove('hidden');
    ...
    async function renderPreviewGrid() {
      ...
      if (previewMode === 'framed') {
        ...
        try {
          const key = 'stripPreview|' + selectedLayout + '|' + currentFrame + '|' + currentFilter + '|' + capturedImages.join('|').slice(-200);
    ...
    /* ---------- Idle auto-reset (disabled) ----------
       This used to auto-reset the studio back to the welcome screen after 60s
       of no interaction (handy for a public/event kiosk that needs to reset
       itself between guests). On a personal setup it just meant the studio
       would unexpectedly bail out to the welcome screen while you were still
       reading/thinking, so it's turned off — arm/clear stay as no-ops so every
       existing armIdleTimer()/clearIdleTimer() call site elsewhere keeps working
       without needing to be touched one by one. */
    function clearIdleTimer() {
      if (idleTimer) { clearTimeout(idleTimer); idleTimer = null; }
    }
    function armIdleTimer() {
      clearIdleTimer();
      // Intentionally not scheduling a reset — see note above.
    }
    ...
    function showQrForDataUrl(fullDataUrl) {
      ...
      img.onload = () => {
        ...
        // CATATAN PENTING: sudah dites langsung pakai JPEG asli — thumbnail
        // SEKECIL APAPUN (bahkan 20x26px, kualitas terendah) tetap nyangkut di
        // sekitar 650-950 karakter base64 karena JPEG punya overhead tetap
        // (tabel Huffman & kuantisasi) yang nggak bisa dikecilin lagi. Jadi
        // datanya udah mepet ke batas minimum yang mungkin — nggak ada lagi
        // yang bisa dihemat dari sisi ukuran data. Yang ternyata jadi masalah
        // utama supaya bisa discan itu UKURAN FISIK kotak QR-nya di layar:
        // modul QR yang cuma ~1.5px kegedean rapat, susah ditangkap fokus
        // kamera HP — apalagi ini di-scan dari layar (bukan cetakan), jadi
        // rawan efek moire. Makanya sekarang kotak QR-nya diperbesar jauh
        // (160px -> 240px) supaya tiap modulnya jauh lebih lebar & jelas,
        // sekalian datanya tetap ditekan sekecil mungkin.
        const MAX_LEN = 900;

        const WIDTHS = [50, 40, 32, 24];
        let thumb = '';
        outer:
        for (const targetW of WIDTHS) {
          ...
          if (thumb.length <= MAX_LEN) break outer;
        ...
        qrCodeBox.innerHTML = '';
        if (thumb.length <= 1150) {
          // Kotak fisiknya digedein jauh (240px) — di versi QR moderat (~15-18)
          // yang dihasilkan dari data sekecil ini, tiap modul jadi ~2.7-3px,
          // jauh lebih gampang ditangkap fokus kamera dibanding sebelumnya.
          new QRCode(qrCodeBox, { text: thumb, width: 240, height: 240, colorDark: '#22261c', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.L });
          qrPanelNote.textContent = 'Pratinjau kecil & buram (biar tetap gampang di-scan). Unduh tombol di atas buat file resolusi penuh.';
        } else {
          qrCodeBox.innerHTML = '<div class="w-[240px] h-[240px] flex items-center justify-center text-center text-[10px] text-[#6b6152] px-2 leading-snug">QR tak muat untuk foto ini</div>';
          qrPanelNote.textContent = 'Foto ini terlalu detail buat kode QR yang gampang discan. Unduh tombol di atas buat simpan file resolusi penuh.';
        }
    ...
    // Dipanggil oleh tombol "Tampilkan QR" — muncul begitu semua pose kelar difoto,
    // nggak perlu nunggu di-download dulu. Kalau strip belum pernah diunduh di sesi
    // ini, generate dulu dari foto yang sudah ada (buildPrintStripDataUrl), baru
    // dipakai buat QR-nya; kalau sudah pernah diunduh, pakai file yang sama persis.
    async function revealQrPanel() {
      if (lastExportedDataUrl) {
        showQrForDataUrl(lastExportedDataUrl);
        return;
      }
      if (capturedImages.length < POSE_COUNT) {
        showToast('Selesaikan sesi foto dulu ya, baru bisa tampilin QR-nya.', 'ℹ️');
        return;
      }
      try {
        if (qrCodeBox) qrCodeBox.innerHTML = '<div class="w-[240px] h-[240px] flex items-center justify-center"><i class="fa-solid fa-spinner fa-spin text-[#8f8163] text-xl"></i></div>';
        if (qrPanel) qrPanel.classList.remove('hidden');
        const dataUrl = await buildPrintStripDataUrl();
        showQrForDataUrl(dataUrl);
      } catch (e) {
        showToast('Gagal bikin QR, coba lagi ya.', '⚠️');
    ...
    /* Renders the SAME strip artwork used for the download, at every stage —
       ...
       Layout-aware: when the selected layout's exportMode is "grid" (Grid
       2×2), slots are arranged in a 2-column grid instead of one vertical
       column — this is what actually drives the on-screen "Slot Cetak"
       preview, so simply toggling #photo-list's CSS class (mockup UI) was
       NOT enough on its own: this canvas render replaces that mockup as
       soon as it's ready and previously always laid out vertically no
       matter which layout was chosen. */
    function buildPrintStripDataUrl() {
      return new Promise((resolve, reject) => {
        ...
        const isGridLayout = (LAYOUT_PRESETS[selectedLayout] || {}).exportMode === 'grid';
        const cols = isGridLayout ? 2 : 1;
        const rows = Math.ceil(N / cols);
        const W = photoW * cols + (gap * (cols - 1)) + (pad * 2);
        const H = (pad * 2) + (photoH * rows) + (gap * (rows - 1)) + footerH;

        // Position of the idx-th photo slot: single column when strip4/strip6,
        // 2-column grid when grid4.
        ...
        function finalize() {
          // Corner sticker flourishes assume a single-column strip and would
          // land oddly across a 2-column grid, so skip them for grid layout —
          // everything else (border, stamp, footer) still applies identically.
          if (!isGridLayout) drawThemeStickersOnCanvas(ctx, W, H, photoW, photoH, pad, gap);
        ...
        for (let idx = 0; idx < N; idx++) {
          if (idx >= capturedImages.length) {
            const { x: destX, y: destY } = slotPos(idx);
