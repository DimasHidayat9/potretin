const video = document.getElementById('webcam');
    ...
    async function exportPhotoStrip() {
      if (capturedImages.length < POSE_COUNT) return;
      try {
        ...
        if (window.JepretinAuth) JepretinAuth.addHistoryEntry(dataUrl, filename, 'strip');
        celebrateOutput();
        showQrForDataUrl(dataUrl);
        showToast('Foto berhasil diunduh!', '🖼️');
      } catch (e) {
        showToast('Gagal membuat strip foto.', '⚠️');
      }
    }
      ...
      capturedImages.forEach((src, idx) => {
        ...
        img.onload = () => {
          ...
          if (loaded === N) {
            ...
            if (window.JepretinAuth) JepretinAuth.addHistoryEntry(dataUrl, filename, 'grid');
            celebrateOutput();
            showQrForDataUrl(dataUrl);
    ...
    async function exportSingleIGPost() {
      if (favoriteIndex === null || !capturedImages[favoriteIndex]) {
      ...
      if (window.JepretinAuth) JepretinAuth.addHistoryEntry(dataUrl, filename, 'ig');
      celebrateOutput();
      ...
      try {
        ...
        gifshot.createGIF({
          ...
          if (!obj.error) {
            ...
            if (window.JepretinAuth) JepretinAuth.addHistoryEntry(obj.image, filename, 'gif');
            celebrateOutput();
